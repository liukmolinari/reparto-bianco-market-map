(() => {
  ...
  const CHAIN_SHORT = {MediaWorld:'MW',Unieuro:'UE',Euronics:'EU',Trony:'TR',Comet:'CO'};
  ...
  function makeChip(label, attrs={}){
    ...
    Object.entries(attrs).forEach(([k,v])=>b.dataset[k]=v); return b;
  }
  ...
  document.addEventListener('click',e=>{
    const b=e.target.closest('[data-chain],[data-zone],[data-priority]'); if(!b)return;
  ...
  function markerIcon(store){
    const cls=`marker-${CHAINS.includes(store.chain)?store.chain:'Custom'}`;
    return L.divIcon({className:'',html:`<div class="chain-marker ${cls}">${CHAIN_SHORT[store.chain]||'+'}</div>`,iconSize:[34,34],iconAnchor:[17,17]});
  }
  ...
  function render(){
    ...
    visibleCount.textContent=stores.length; zoneLabel.textContent=activeZone; chainLabel.textContent=activeChain==='ALL'?'Tutte':activeChain;
  }
  map.on('popupopen',()=>{
  ...
  function removeStore(id){
    ...
    removed.add(id); localStorage.setItem(REMOVED_KEY,JSON.stringify([...removed])); map.closePopup(); render();
  }

  // Long press/click helper coordinates
  map.on('contextmenu',e=>L.popup().setLatLng(e.latlng).setContent(`<b>Coordinate</b><br>Lat ${e.latlng.lat.toFixed(6)}<br>Lng ${e.latlng.lng.toFixed(6)}`).openOn(map));
  ...
  CHAINS.forEach(c=>fChain.add(new Option(c,c))); fChain.add(new Option('Altro','Custom'));
  REGIONS.forEach(r=>fRegion.add(new Option(r,r)));
  function openModal(){const c=map.getCenter();document.getElementById('fLat').value=c.lat.toFixed(6);document.getElementById('fLng').value=c.lng.toFixed(6);modal.classList.remove('hidden');backdrop.classList.remove('hidden')}
  function closeModal(){modal.classList.add('hidden');backdrop.classList.add('hidden')}
  document.getElementById('addStoreFab').onclick=openModal; document.getElementById('closeAdd').onclick=closeModal; backdrop.onclick=closeModal;
  document.getElementById('useMapCenter').onclick=()=>{const c=map.getCenter();document.getElementById('fLat').value=c.lat.toFixed(6);document.getElementById('fLng').value=c.lng.toFixed(6)};
  document.getElementById('addStoreForm').onsubmit=e=>{
    e.preventDefault(); const region=fRegion.value;
    const s={id:'custom-'+Date.now(),chain:fChain.value,name:document.getElementById('fName').value.trim(),city:document.getElementById('fCity').value.trim(),region,zone:REGION_ZONE[region]||'Italia',priority:document.getElementById('fPriority').value,opFit:Number(document.getElementById('fOpFit').value)||0,comFit:Number(document.getElementById('fComFit').value)||0,top25:false,lat:Number(document.getElementById('fLat').value),lng:Number(document.getElementById('fLng').value),notes:document.getElementById('fNotes').value.trim(),custom:true};
    customStores.push(s); localStorage.setItem(STORE_KEY,JSON.stringify(customStores)); closeModal(); e.target.reset(); render(); map.setView([s.lat,s.lng],12);
  };

  // Import/export/reset
  document.getElementById('exportData').onclick=()=>{
    const data={version:1,customStores,removed:[...removed]}; const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'}); const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='reparto-bianco-market-map-backup.json';a.click();URL.revokeObjectURL(a.href);
  };
  document.getElementById('importData').onchange=async e=>{
    const file=e.target.files[0]; if(!file)return; try{const data=JSON.parse(await file.text());customStores=Array.isArray(data.customStores)?data.customStores:[];removed=new Set(Array.isArray(data.removed)?data.removed:[]);localStorage.setItem(STORE_KEY,JSON.stringify(customStores));localStorage.setItem(REMOVED_KEY,JSON.stringify([...removed]));render();alert('Dati importati.')}catch(err){alert('File non valido.')} e.target.value='';
  };
  document.getElementById('resetCustom').onclick=()=>{
    if(!confirm('Ripristinare la mappa iniziale e cancellare modifiche locali?'))return; customStores=[];removed.clear();localStorage.removeItem(STORE_KEY);localStorage.removeItem(REMOVED_KEY);render();
  };
